...
<%
    ...
    String sql = "SELECT *FROM t_movie  where movie_name  like '%"+kd+"%' or movie_type  like '%"+kd+"%'or director like '%"+kd+"%'  or roles like '%"+kd+"%'";
    try {
...
<form class="form-inline definewidth m20" >
    <button type="button" class="btn btn-success btn-sm"><a href="movieList.jsp" style="text-decoration:none;color: #FFFFFF;font-weight: bold">返回</a></button>
</form>
...
<table class="table table-bordered table-hover definewidth m10" style="table-layout:fixed;">
    ...
    <tr>
        ...
        <td> <a href="../movie/detailmovie.jsp?moviename=<%=movie.getMovie_name()%>"name="moviename"><%=movie.getMovie_name()%>
        </td>
        ...
        <td>
            <button type="submit" style="float: left;text-align: left" class="btn btn-warning btn-xs">编辑</button>
            <button type="submit" style="float: right;text-align: right" class="btn btn-danger btn-xs">删除</button>
        </td>
    </tr>
    <%
        }
